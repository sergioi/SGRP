...
class ValorHelpers
{
    ...
    public static function activarlaEtapa1($estado_nombre) //recibo inactivo
    {
        $userTieneEstadoName = Activacion::find('id')
        ->where(['activacion1' => $estado_nombre])
        ->one();
        return isset($userTieneEstadoName -> id) ? true : false;
    }


    public static function activarlaEtapa2($estado_nombre) //recibo inactivo
    {
        $userTieneEstadoName = Activacion::find('id')
        ->where(['activacion2' => $estado_nombre])
        ->one();
        return isset($userTieneEstadoName -> id) ? true : false;
    }

    public static function activarlaEtapa3($estado_nombre) //recibo inactivo
    {
        $userTieneEstadoName = Activacion::find('id')
        ->where(['activacion3' => $estado_nombre])
        ->one();
        return isset($userTieneEstadoName -> id) ? true : false;
    }

    public static function activarlaEtapa4($estado_nombre) //recibo inactivo
    {
        $userTieneEstadoName = Activacion::find('id')
        ->where(['activacion33' => $estado_nombre])
        ->one();
        return isset($userTieneEstadoName -> id) ? true : false;
    }
